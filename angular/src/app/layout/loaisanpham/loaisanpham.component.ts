...
export class LoaisanphamComponent implements OnInit {
    ...
    getSingled(id: string) {
        this.loaisanphamService.getItem(id).subscribe((res) => {
            this.entity = res;
        });
    }
    ...
    onClick(value: any) {
        ...
        if (this.Id == 0) {
            ...
            console.log(value);
            this.loaisanphamService.editItem(value.mahang, value).subscribe((res) => {
                alert('edit success!');
                this.AddEditModal.hide();
                this.loadData();
            });
    ...
    onDelete(id: string) {
        ...
        if (confirmResult) {
            this.loaisanphamService.deleteItem(id).subscribe((response) => {
                if (response) {
